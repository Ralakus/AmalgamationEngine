	...
	try {
		...
		while (Window->IsValid()) {
			...
			if (Window->IsMouseLocked()) {
				...
				Cam->Pitch(Cam->GetMouseDelta().y * T.GetDelta() * (InvertMouseY ? -1 : 1));
				Cam->Yaw(Cam->GetMouseDelta().x * T.GetDelta() * (InvertMouseX ? -1 : 1));
			}
...
class Sausage : public TComponent<Sausage> {
public:
	uint32 BytesTaken;
	float Length;
};

class SausageSystem : public ISystem {
public:
	SausageSystem() : ISystem({ Sausage::ID }) {}
	void UpdateComponents(float Delta, IComponent** Components) {
		Sausage& CurSause = *reinterpret_cast<Sausage*>(*Components);
		if (CurSause.BytesTaken > 0) {
			std::cout << "Sausage " << rang::fgB::cyan << &CurSause << rang::fg::reset << " has "
				<< rang::fgB::red << CurSause.BytesTaken << rang::fg::reset << " bites taken and is " <<
				rang::fgB::green << CurSause.Length << rang::fg::reset << " cm long" << std::endl;
		}
	}

	void TakeByte(EntityHandle Entity, Scene& S) {
		Sausage* Bitten = S.GetComponent<Sausage>(Entity);
		if (Bitten) {
			Bitten->BytesTaken++;
			Bitten->Length--;
		}
...
int main(int argc, char* args[]) {
	...
	SausageSystem SauseSystem;
	CurScene.AddSystem(SauseSystem);

	Sausage SauseComp;
	SauseComp.Length = 10.f;
	EntityHandle Sause = CurScene.CreateEntity(nullptr, nullptr, 0);
	CurScene.AddComponent<Sausage>(Sause, &SauseComp);
	CurScene.GetComponent<Sausage>(Sause)->BytesTaken = 0;
	CurScene.GetComponent<Sausage>(Sause)->Length = 10.f;

	InputControl BiteSausage;
	BiteSausage.AddInput(Key::F, 1.f);

	while (RenderWindow.IsValid()) {
		RenderWindow.Update();
		CurScene.UpdateSystems(0.f);

		if (BiteSausage.Value() > 0) {
			SauseSystem.TakeByte(Sause, CurScene);
		}
